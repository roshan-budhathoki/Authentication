<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($username) || empty($password)){
            ...
            $query = mysqli_query($connect, "SELECT * FROM users WHERE username = '$username' AND password = '$hassedPassword'");
            ...
            if(mysqli_num_rows($query) > 0){
                ...
                //store into data base {YOU ARE STORING USER ID INTO THE COOKIES } So {USER ID IS SAME AS COOKIE VALUE}  THATS WHY {WE WILL STORE USRE ID AND CREATE COOKIE ACCORDING TO ID}
                    ...
                    $checker = mysqli_query($connect, "INSERT INTO products (user_number, code) VALUES ('{$result['user_id']}', '$randomValue')");
